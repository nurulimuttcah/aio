...
# //====================================================
sleep 3
# // FONT color configuration | AUTOSCRIPT-LYNZ
Green="\e[92;1m"
RED="\033[31m"
YELLOW="\033[33m"
BLUE="\033[36m"
...
# // configuration GET | AUTOSCRIPT-LYNZ
# // configuration GET |  LYNZ SC AIO
TIMES="10"
NAMES=$(whoami)
IMP="wget -q -O"
CHATID="1423578532"
...
KEY="6091152473:AAHj6utHcce1nit-cXNavOazBCfUI3iM4fk"
URL="https://api.telegram.org/bot$KEY/sendMessage"
GITHUB_CMD="https://github.com/nurulimuttcah/aio/permission/raw/"
...
judge() {
    if [[ 0 -eq $? ]]; then
        print_ok "$1 Complete... | thx to ${YELLOW}alhamdulillah S.W.T${FONT}"
        sleep 1
    fi
}
...
function LOGO() {
 ...
 ───│    $Green├─┤│ │ │ │ │└─┐│  ├┬┘│├─┘ │   │  │ │ ├┤ $NC   │───
 ───│    $Green┴ ┴└─┘ ┴ └─┘└─┘└─┘┴└─┴┴   ┴   ┴─┘┴ ┴ └─┘$NC   │───
    │    ${YELLOW}copas${FONT} (C)$GRAY https://t.me/amantubilah $NC   │
    └───────────────────────────────────────────────┘
${RED}Autoscript-anggun   (multi port) Gratis${FONT}    


}
function install_xray() {
    judge "Core Xray lates Version installed successfully"
    curl -s ipinfo.io/city >>/etc/xray/city
    curl -s ipinfo.io/org | cut -d " " -f 2-10 >>/etc/xray/isp
    bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install -u www-data --version lates
...
function download_config() {
    ...
    wget ${GITHUB_CMD}main/fodder/nginx/bahan.zip >/dev/null 2>&1
...
END
    ...
    wget -q -O /etc/bannermu.txt "${GITHUB_CMD}main/fodder/FighterTunnel-examples/banner" >/dev/null 2>&1
...
function configure_nginx() {
    ...
    sed -i -e 's/\r$//' /usr/bin/vmess
    sed -i -e 's/\r$//' /usr/bin/vless 
    sed -i -e 's/\r$//' /usr/bin/trojan
    sed -i -e 's/\r$//' /usr/bin/gotop
    sed -i -e 's/\r$//' /usr/bin/bkp
    
    chown -R www-data:www-data /etc/msmtprc
    source <(curl -sL ${GITHUB_CMD}main/fodder/FighterTunnel-examples/Documentation/tunip)
